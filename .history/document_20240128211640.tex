...
\hypersetup{
    colorlinks=false,    % 设置为 false 以使用边框而不是颜色链接
    %linkbordercolor={1 0 0}, % 设置红色边框（RGB格式）
    pdfborderstyle={/S/S/W 2} % 设置边框样式，U 为下划线，S 为实线，W 为边框宽度
}
...
\subsection{图表的创建和引用}
    The above data is combined to form a correlation heat map bettween features,as show in Fig.\fig_ref{fig:}{fig:time-freq}
    \begin{figure}[h]
        \centering
